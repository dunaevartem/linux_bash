auto_archive.sh
auto_archive.sh – это небольшая Bash‑утилита, созданная для того, чтобы быстро создавать резервные копии выбранного каталога и восстанавливать их по требованию.

Скрипт хранит путь к резервируемой папке в простом конфиг‑файле backup_src, так что после первого запуска вам не нужно каждый раз вводить путь – просто нажимайте ENTER, чтобы оставить прежний.
...
Как запустить
# Скачайте скрипт
curl -L -o auto_archive.sh https://example.com/auto_archive.sh
