...
```
__Что делать, если архив не создаётся__ \
Убедитесь, что в каталоге __./backup__ есть место для записи. Скрипт создаёт этот каталог автоматически (```mkdir -p ./backup```).
Проверьте, что у вас достаточно прав на чтение из исходного каталога и запись в каталог __./backup__ .
В случае ошибок в выводе скрипта будет сообщение «Ошибка при создании резервной копии!».

__Как восстановить__
```Выберите операцию:
1. Архивирование
2. Восстановление
3. Выход
...
__auto_archive.sh__ – простой способ быстро делать резервные копии и восстанавливать их, не усложняя настройку и без сторонних утилит.
